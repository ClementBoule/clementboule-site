...
@keyframes discTileFloat {
  0%,100% { transform: rotate(var(--r)) translateY(0); }
  50%      { transform: rotate(var(--r)) translateY(-5px); }
}
@keyframes shimmer {
...
.hero-name-shimmer {
  background: linear-gradient(90deg, #1A2B4A 40%, #3D6DB8 50%, #6B9ED4 60%, #1A2B4A 70%);
...
function FadeIn({
  ...
  const hidden =
    direction === 'left'  ? { opacity: 0, transform: 'translateX(-40px)', transition }
    : direction === 'right' ? { opacity: 0, transform: 'translateX(40px)',  transition }
    : direction === 'down'  ? { opacity: 0, transform: 'translateY(-24px)', transition }
...
export default function Hero() {
  ...
  return (
    <section
      ...
      style={{
        background: 'linear-gradient(135deg, #EEF3FC 0%, #F5F7FA 40%, #EAF0FB 70%, #F0EEF8 100%)',
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="orb-a absolute -top-24 -right-24 w-[500px] h-[500px] rounded-full"
          style={{ background: 'radial-gradient(circle, #F5A98C44 0%, transparent 70%)' }} />
        {/* Blue orb left */}
        <div className="orb-b absolute top-1/3 -left-32 w-[420px] h-[420px] rounded-full"
          style={{ background: 'radial-gradient(circle, #6B9ED444 0%, transparent 70%)' }} />
        {/* Purple orb center-bottom */}
        <div className="orb-c absolute bottom-0 left-1/2 -translate-x-1/2 w-[380px] h-[280px] rounded-full"
          style={{ background: 'radial-gradient(circle, #B09FE530 0%, transparent 70%)' }} />

        {/* Small floating geometric accents */}
        <FloatingShape
          className="absolute top-[18%] left-[8%] w-3 h-3 rounded-sm opacity-25"
          style={{ background: '#3D6DB8', transform: 'rotate(15deg)', animation: 'floatA 5s ease-in-out infinite' }}
        />
        <FloatingShape
          className="absolute top-[35%] right-[12%] w-4 h-4 rounded-sm opacity-20"
          style={{ background: '#F5A98C', transform: 'rotate(-10deg)', animation: 'floatB 6s ease-in-out infinite .5s' }}
        />
        <FloatingShape
          className="absolute bottom-[30%] left-[15%] w-2.5 h-2.5 rounded-full opacity-30"
          style={{ background: '#16A34A', animation: 'floatC 7s ease-in-out infinite 1s' }}
        />
        <FloatingShape
          className="absolute top-[60%] right-[20%] w-2 h-2 rounded-full opacity-25"
          style={{ background: '#EAB308', animation: 'floatA 4.5s ease-in-out infinite 2s' }}
        />
        <FloatingShape
          className="absolute bottom-[20%] right-[8%] w-3 h-3 opacity-20"
          style={{ background: '#DC2626', borderRadius: '2px', transform: 'rotate(30deg)', animation: 'floatB 5.5s ease-in-out infinite .8s' }}
        />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-28 pb-20">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-5">
            ...
            <FadeIn direction="left" delay={80}>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold leading-none tracking-tight hero-name-shimmer">
                Clément Boulé
              </h1>
            </FadeIn>
            ...
            <FadeIn direction="left" delay={200}>
              <div className="inline-flex items-center gap-2">
                <span className="w-8 h-[2px] rounded-full bg-gradient-to-r from-[#3D6DB8] to-[#6B9ED4]" />
            ...
            <FadeIn direction="left" delay={440}>
              <div className="flex flex-wrap items-center gap-4 pt-3">
                <a
                  href="/test-disc"
                  className="inline-flex items-center gap-3 border border-[#1A2B4A]/15 hover:border-[#3D6DB8]/40 bg-white/70 hover:bg-white font-medium px-5 py-3 rounded-full transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#3D6DB8]/10 group backdrop-blur-sm"
                >
                  <DiscLogo size={80} />
                  <span className="text-[#1A2B4A]/70 group-hover:text-[#3D6DB8] transition-colors">{t.hero.disc}</span>
                </a>
              </div>
            </FadeIn>
            ...
            <FadeIn direction="left" delay={560}>
              <div className="flex items-center gap-4 pt-2">
                <a
                  ...
                  className="inline-flex items-center gap-2 text-sm text-[#0A66C2] hover:text-[#004182] font-medium transition-all hover:-translate-y-0.5"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-sm text-[#FC5656] hover:text-[#E03E3E] font-medium transition-all hover:-translate-y-0.5"
                >
          ...
          <FadeIn direction="right" delay={150} className="relative flex justify-center md:justify-end">
            ...
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, #6B9ED455 0%, #F5A98C40 50%, #B09FE540 100%)',
            ...
            {/* Decorative dot grid */}
            <div
              className="absolute -bottom-6 -right-6 w-32 h-32 opacity-25"
              style={{
                backgroundImage: 'radial-gradient(circle, #3D6DB8 1.5px, transparent 1.5px)',
        ...
        {/* Scroll indicator */}
        <FadeIn delay={900} direction="up" className="absolute bottom-6 left-1/2 -translate-x-1/2">
          <div className="flex flex-col items-center gap-1 text-[#1A2B4A]/30 text-xs">
            <div
              className="w-[1px] h-8 bg-gradient-to-b from-[#3D6DB8]/50 to-transparent"
              style={{ animation: 'floatC 2s ease-in-out infinite' }}
            />
          </div>
        </FadeIn>
      </div>
