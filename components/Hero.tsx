          ...
          <div className="space-y-5">
            ...
            <FadeIn direction="left" delay={560}>
              <div className="flex items-center gap-4 pt-2">
                ...
                >
                  <img src="/logos/linkedin.png" alt="LinkedIn" style={{height:'20px',width:'auto',objectFit:'contain'}} onError={(e) => {
    const t = e.currentTarget;
    const src = t.getAttribute('data-src') || t.src;
    if (!t.getAttribute('data-src')) t.setAttribute('data-src', src);
    if (src.endsWith('.png')) { t.src = src.replace('.png', '.jpg'); }
    else if (src.endsWith('.jpg')) { t.src = src.replace('.jpg', '.jpeg'); }
    else {
      t.style.display='none';
      const p = t.parentElement;
      if (p && !p.querySelector('.lph')) {
        const d = document.createElement('div');
        d.className='lph';
        d.style.cssText='width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#f3f4f6;border:2px dashed #d1d5db;border-radius:8px;font-size:11px;color:#9ca3af;';
        d.textContent='Logo à uploader';
        p.appendChild(d);
      }
    }
  }} />
