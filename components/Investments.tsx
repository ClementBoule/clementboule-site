...
const INVESTMENTS: { name: string; logo: string; product: string; productAlt: string; productAltEn: string; url: string; tagline: string; taglineEn: string; rot: string; shadowColor: string; tilt: string }[] = [
  { name: 'Daan Tech',  logo: '/logos/inv-daan-tech.png', product: '/logos/prod-daan-bob.jpg',     productAlt: 'Bob, lave-vaisselle compact',  productAltEn: 'Bob, compact dishwasher',     url: 'https://www.daan.tech/',  tagline: 'Hardware FR Â· lave-vaisselle Bob',  taglineEn: 'French hardware Â· Bob dishwasher',     rot: '-rotate-[0.5deg]', shadowColor: 'var(--cb-sarcelle)',   tilt: 'rotate-[8deg]' },
  { name: 'Mosa Meat',  logo: '/logos/inv-mosa-meat.png', product: '/logos/prod-mosa-meat.jpg',  productAlt: 'Burger en viande cultivÃ©e',    productAltEn: 'Cultivated meat burger',      url: 'https://mosameat.com/',   tagline: 'Viande cultivÃ©e Â· Pays-Bas',         taglineEn: 'Cultivated meat Â· Netherlands',         rot: 'rotate-[0.4deg]',  shadowColor: 'var(--cb-terracotta)', tilt: '-rotate-[6deg]' },
  { name: 'My Food',    logo: '/logos/inv-my-food.png',   product: '/logos/prod-myfood-serre.jpg', productAlt: 'Serre connectÃ©e My Food',      productAltEn: 'My Food connected greenhouse', url: 'https://www.myfood.eu/',  tagline: 'Agriculture urbaine connectÃ©e',      taglineEn: 'Connected urban farming',                rot: '-rotate-[0.3deg]', shadowColor: 'var(--cb-sauge-deep)', tilt: 'rotate-[10deg]' },
]
...
function InvestmentCard({
  ...
  return (
    <a
      ...
      aria-label={`${name} Â· ${lang === 'fr' ? tagline : taglineEn}`}
    >
      <img
        src={logo}
        alt={name}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-10 max-w-3xl">
          ...
          <p className="text-base md:text-lg max-w-2xl font-medium leading-relaxed text-pretty">
            {lang === 'fr' ? (
              <>Petit actionnaire, en direct, de trois sociÃ©tÃ©s qui m'ont convaincu sur leur transition. Hardware franÃ§ais, viande cultivÃ©e, agriculture urbaine.{' '}<strong className="bg-cb-rose px-1.5 rounded-sm font-semibold">De quoi rester proche des arbitrages rÃ©els que les dirigeants me racontent en mission.</strong></>
            ) : (
